...
export default function MePage() {
    ...
    const startEdit = () => {
        ...
        setEmail(person.email ?? userEmail)
    ...
    const myLevel      = 0  // always 0 in lineage
    ...
    // Build generation timeline from lineage (level 0 = self, 1 = parents, 2 = grandparents…)
    // Group by level, show highest levels first (grandparents → parents → me)
    const genTimeline = (() => {
        if (!Array.isArray(lineage) || lineage.length === 0) return []
        const byLevel: Record<number, LineagePerson[]> = {}
        for (const l of lineage) {
            byLevel[l.level] = byLevel[l.level] ? [...byLevel[l.level], l] : [l]
        }
        const levels = Object.keys(byLevel).map(Number).sort((a, b) => b - a) // high level first
        const maxLvl = levels[0]
        return levels.map(lvl => ({
            level: lvl,
            genNum: maxLvl - lvl + 1,
            names: byLevel[lvl].map(l => l.person.first_name).join(", "),
    ...
    return (
        <div style={{ display: "flex", height: "100vh", width: "100vw", overflow: "hidden" }}>
            <AppSidebar activeSection="profile" activeFamilyId={person.family_id ?? undefined} />
                            ...
                            <div style={{ flex: 1, position: "relative", zIndex: 1 }}>
                                    ...
                                    <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                                        <input
                                            ...
                                            style={{
                                                ...
                                                borderRadius: 10, padding: "6px 14px", outline: "none",
                                                flex: 1,
                                            }}
                                        />
                                        <input
                                ...
                                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                                    ...
                                    {person.birth_date && (
                                        <div style={{
                                            display: "inline-flex", alignItems: "center", gap: 7,
                                            padding: "6px 16px", background: "rgba(255,255,255,0.15)", borderRadius: 20,
                                        }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                                        ...
                                        <InfoRow icon={Mail} label="Email">
                                            {editing ? (
                                                ...
                                                <span>{person.email || userEmail || "—"}</span>
                                            )}
                                        </InfoRow>
